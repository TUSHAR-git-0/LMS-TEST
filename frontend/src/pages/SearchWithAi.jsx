  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black to-gray-900 text-white flex flex-col items-center px-4 py-16">
      ...
      <div className="bg-white shadow-xl rounded-3xl p-6 sm:p-8 w-full max-w-2xl text-center relative">
        ...
        <div className="flex items-center bg-white border-2 border-gray-200 rounded-full overflow-hidden shadow-lg relative w-full ">
          
          <input
            type="text"
            className="flex-grow px-4 py-3 bg-transparent text-gray-900 placeholder-gray-500 focus:outline-none text-sm sm:text-base"
            placeholder="What do you want to learn? (e.g. AI, MERN, Cloud...)"
            value={input}
            onChange={(e) => setInput(e.target.value)}
            onKeyDown={(e) => { if (e.key === "Enter" && input.trim()) handleRecommendation(input.trim()); }}
          />
          ...
          {input && (
            <button
              ...
              className="absolute right-14 sm:right-16 bg-gray-100 rounded-full"
            >
          ...
          <button
            className="absolute right-2 bg-gray-100 rounded-full w-10 h-10 flex items-center justify-center"
      ...
      {recommendations.length > 0 ? (
        ...
        listening? <h1 className='text-center text-xl sm:text-2xl mt-10 text-gray-300'>Listening...</h1>:<h1 className='text-center text-xl sm:text-2xl mt-10 text-gray-300'>Type above or press the mic to search courses</h1>
