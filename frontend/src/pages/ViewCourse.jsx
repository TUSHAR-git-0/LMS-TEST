...
const handleEnroll = async (courseId, userId) => {
  try {
    ...
    const price = Number(selectedCourseData?.price);
    if (!price || price <= 0) {
      ...
      const refreshedUser = await axios.get(serverUrl + "/api/user/currentuser", { withCredentials: true });
      dispatch(setUserData(refreshedUser.data));
...
setIsEnrolled(true)
  ...
  return (
     <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-6xl mx-auto bg-white shadow-xl rounded-3xl p-6 space-y-6 relative animate-fade-up border border-gray-50">
        ...
        <div className="flex flex-col md:flex-row gap-6 ">
          ...
          <div className="w-full md:w-1/2">
            ...
            {selectedCourseData?.thumbnail ? <img
              src={getFullUrl(selectedCourseData?.thumbnail)}
          ...
          <div className="flex-1 space-y-2 mt-[20px]">
            ...
            {/* Enroll Button */}
            {!isEnrolled ?<button className="shimmer-btn bg-gradient-to-r from-violet-600 to-fuchsia-500 text-white px-7 py-2.5 rounded-full font-semibold hover:shadow-lg hover:shadow-violet-300 hover:scale-105 transition-all active:scale-95 mt-3 cursor-pointer" onClick={()=>handleEnroll(courseId , userData._id)}>
              Enroll Now
            </button> :
            <button className="bg-emerald-50 text-emerald-600 px-7 py-2.5 rounded-full font-semibold hover:bg-emerald-100 hover:scale-105 transition-all active:scale-95 mt-3 cursor-pointer" onClick={()=>navigate(`/viewlecture/${courseId}`)}>
             Watch Now
            </button>
            }
          </div>
  ...
  <div className="bg-white w-full md:w-3/5 p-6 rounded-2xl shadow-lg border border-gray-200">
    <div className="aspect-video w-full rounded-lg overflow-hidden mb-4 bg-black flex items-center justify-center">
      {selectedLecture?.videoUrl ? (
        <video
          src={getFullUrl(selectedLecture.videoUrl)}
          controls
...
<div className="mt-8 border-t pt-6">
    ...
    <div className="mb-4">
      ...
      <button
        ...
        className="shimmer-btn bg-gradient-to-r from-violet-600 to-fuchsia-500 text-white mt-3 px-6 py-2.5 rounded-full font-semibold cursor-pointer hover:scale-105 hover:shadow-lg hover:shadow-violet-300 transition-all active:scale-95" onClick={handleReview}
      >
        Submit Review
      </button>
    </div>
        ...
        <div className="flex items-center gap-4 pt-4 border-t ">
          {creatorData?.photoUrl ?<img
            src={getFullUrl(creatorData?.photoUrl)}
            alt="Instructor"
        ...
        <div>
          <p className='text-xl font-semibold mb-2'>More courses from this <span className="gradient-text">Educator</span> -</p>
