...
function ViewLecture() {
  ...
  return (
<div className="min-h-screen bg-gradient-to-br from-gray-50 to-violet-50/40 p-6 flex flex-col md:flex-row gap-6">
      
      {/* Left - Video & Course Info */}
      <div className="w-full md:w-2/3 bg-white rounded-2xl shadow-md p-6 border border-gray-100 card-lift">
        {/* Course Details */}
        <div className="mb-6" >
              ...
              <button
                ...
                className={`flex items-center justify-between p-3 rounded-xl border transition-all text-left ${
                  selectedLecture?._id === lecture._id
                    ? 'bg-gradient-to-r from-violet-50 to-fuchsia-50 border-violet-400 shadow-md'
                    : 'hover:bg-gray-50 border-gray-200 hover:border-violet-200'
                }`}
              >
                <div>
                  <h4 className={`text-sm font-semibold ${selectedLecture?._id === lecture._id ? "text-violet-700" : "text-gray-800"}`}>{lecture.lectureTitle}</h4>
                  
                </div>
                <FaPlayCircle className={`text-xl ${selectedLecture?._id === lecture._id ? "text-violet-600" : "text-gray-400"}`} />
              </button>
            ))
          ) : (
  ...
  <div className="mt-4 border-t pt-4">
    ...
    <div className="flex items-center gap-4">
      <img
        src={getFullUrl(courseCreator.photoUrl) || '/default-avatar.png'}
        alt="Instructor"
