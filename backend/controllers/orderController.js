...
export const createOrder = async (req, res) => {
  try {
    ...
    if (!price || price <= 0) {
      return res.status(400).json({ message: "This is a free course. You can enroll directly without payment." });
    }

    const options = {
      amount: Math.round(price * 100), // in paisa
      currency: 'INR',
...
export const verifyPayment = async (req, res) => {
  try {
        ...
        // Verify signature with HMAC SHA-256
